...
function crearClaseMascota() {
  class Mascota {
    ...
    addActividad(actividad, frecuencia) {
      ...
      // Tu código aca:
      let array = { actividad: actividad , frecuencia: frecuencia }
      this.actividades.push(array)
    }
    ...
    getActividades() {
      ...
      this.actividades.forEach(i => {
        arreglo.push(i.actividad)      
      })
      return arreglo
    }
    ...
    getFrecuencia(actividad) {
      ...
      let 
        valor = [],
        activity = this.actividades
        
      for(let i = 0; i < activity.length; i++) {
        if(actividad == activity[i].actividad){
          valor.push(activity[i].frecuencia)          
        }        
      }
      return valor[0]
    }
